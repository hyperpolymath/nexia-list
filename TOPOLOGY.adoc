...
Nexia-List is *web-first*: the browser is the primary target, running
the real Rust core compiled to WebAssembly. The core carries the *λδ
(LambdaDelta)* substrate — a homoiconic Clojure-flavoured Lisp built
in-tree, not vendored — which is the single largest subsystem in the
core. The desktop shell is optional and depends on an *external* sibling
checkout of `+hyperpolymath/gossamer+`; it is intentionally not built in
this repo’s CI.
...
Future (not yet implemented, kept out of the diagram deliberately):
petgraph graph engine, tantivy full-text search, Nickel schemas, mobile
targets, and the FL×DT intelligence modules (`+index.rs+`, `+edge.rs+`,
`+reason.rs+`, `+layout.rs+`, `+trigger.rs+`) described in docs/design/.
...
`+Cargo.lock+` exactly* (`+just doctor+` checks this); a mismatch fails
the browser build with a confusing schema error.
...
*Percentages are judgement, LOC and test counts are not.* When you
revise a row, re-derive the countable facts rather than copying them
forward — this file understated the core for two weeks because the λδ
merges (#35, #36, #43) landed without a dashboard update, and downstream
docs inherited the stale figures.
