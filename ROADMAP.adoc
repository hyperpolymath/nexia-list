...
. Build resurrection, then estate Bun migration (ReScript compile, Bun bundle) — ✅ done
...
. Tests beyond the core unit suite — ✅ done (11 UI tests incl. TEA↔WASM and complete
  ReScript↔WASM facade contract tests)
...
. Bind the dormant WASM exports in ReScript — ✅ done (31 of 31 bound, including
  `lambdadeltaEval`, `evalLambdadelta`, and `evalFormula`)
. Give λδ its first door — ✅ L1 note-scoped, read-only formula panel shipped;
  source stays collapsed until the user explicitly opens “Computed field (λδ)”

Next sequencing:

. Continue λδ surfacing: L2 action hooks → L3 REPL, gated by progressive
...
|Bun tasks
|setup, dev, build, test, lint, fmt commands
|[x]
